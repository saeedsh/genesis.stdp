static char rcsid[] =
		"$Id: hebbsynchan.c,v 1.4 2005/07/20 20:02:01 svitak Exp $";
...
static void changeStdpWeights(channel, weight_change_mod,
		weight_change_mod_level)
	register struct HebbSynchan_type *channel;unsigned short weight_change_mod;double weight_change_mod_level; {
	int i;
	 ...
	 * the avg_Ca and the nearest threshold, with the appropriate
	 * sign, unless the avg_Ca falls between the thresholds, in which
	...
	if (channel->avg_Ca > channel->post_thresh_hi)
		post_activity = (channel->avg_Ca - channel->post_thresh_hi)
				/ channel->post_scale;
	else if (channel->avg_Ca < channel->post_thresh_lo)
		post_activity = (channel->avg_Ca - channel->post_thresh_lo)
				/ channel->post_scale;
	else
		post_activity = 0.0;
	...
	for (i = 0; i < channel->nsynapses; i++) {
		...
		if (channel->synapse[i].pre_activity > channel->pre_thresh_hi)
			pre_activity = channel->synapse[i].pre_activity
					- channel->pre_thresh_hi;
		else if (channel->synapse[i].pre_activity < channel->pre_thresh_lo)
			pre_activity = channel->synapse[i].pre_activity
					- channel->pre_thresh_lo;
		else
			pre_activity = 0.0;
...
int StdpSynchan(channel, action)
	register struct HebbSynchan_type *channel;Action *action; {
	double x;
	double dt;
	double eventtime;
	...
	short synindex;
	SynapticEvent *event, *next_event, *pending_event, *last_event, *firstnode,
			*lastnode;
	int n;
	int nr;
	...
	double CaArray[AVG_CALCIUM_MAX_SIZE]; //An array to discreetly evaluate Average
			...
			if (channel->synapse[i].mi == spikemsg) {
				...
				if (channel->FreeSynapticEvents == NULL) {
					...
					if (AllocateSynapticEventNodes(
							(struct Synchan_type *) channel, allocnodes,
	...
	case PROCESS:
		...
		if (next_event != NULL) /* skip if no events are pending */
		{
			while ((next_event != NULL) && (next_event->time == 0)) /* execute the event */
			{
				...
				SET_FLAG(SPIKE_THIS_TIME_STEP,
						channel->synapse[syn].spike_flags);
		...
		case AVG_CALCIUM_1: /* calcium */
			CaArray[0] = MSGVALUE(msg, 0);
			break;
		case AVG_CALCIUM_2: /* calcium */
			CaArray[1] = MSGVALUE(msg, 0);
			break;
		case AVG_CALCIUM_3: /* calcium */
			CaArray[2] = MSGVALUE(msg, 0);
			break;
		case AVG_CALCIUM_4: /* calcium */
			CaArray[3] = MSGVALUE(msg, 0);
			break;
		case AVG_CALCIUM_5: /* calcium */
			CaArray[4] = MSGVALUE(msg, 0);
			break;
		case AVG_CALCIUM_6: /* calcium */
			CaArray[5] = MSGVALUE(msg, 0);
			break;
		case AVG_CALCIUM_7: /* calcium */
			CaArray[6] = MSGVALUE(msg, 0);
			break;
		case AVG_CALCIUM_8: /* calcium */
			CaArray[7] = MSGVALUE(msg, 0);
			break;
		...
		case MOD: /* Neuromodulator input */
			...
			if (mod == 0) /* this is the first MOD message */
			{
				...
				/* this is not the first MOD message */
				mod_level *= MSGVALUE(msg, 0);
		...
		case WEIGHT_CHANGE_MOD: /* Modulation of weight_change_rate */
			...
			if (weight_change_mod == 0) /* this is the first WEIGHT_CHANGE_MOD message */
			{
				...
				/* this is not the first WEIGHT_CHANGE_MOD message */
				weight_change_mod_level *= MSGVALUE(msg, 0);
		...
		for (i = 0; i < channel->nsynapses; i++) {
			...
			if (IS_FLAG_SET(SPIKE_THIS_TIME_STEP,
					channel->synapse[i].spike_flags)) {
				x = channel->synapse[i].X * channel->pre_xconst + 1.0;
				CLEAR_FLAG(SPIKE_THIS_TIME_STEP,
						channel->synapse[i].spike_flags);
			} else
			...
			channel->synapse[i].pre_activity = channel->synapse[i].Y
					* channel->pre_norm;
		}
		...
		/*
		 * Calculate the "averaged" postsynaptic Ca; in reality all
		 * we do is use the Vm as input to a leaky integrator with
		 * a time constant of post_tau.
		 *
		 * avg_Ca = (1 - exp(dt/post_tau)) * Ca + exp(dt/post_tau) * avg_Ca
		 * or
		 * avg_Ca = (1 - post_const)*Ca + post_const*avg_Ca
		 *
		 * Note that when time step is changed you must do a RECALC to
		 * recalculate post_const; this is done automatically on a RESET.
		 */
		int cai = 1;
		if (channel->avg_Ca_ExtraMsgNumber > 0) {
			Ca = CaArray[0];
			for (; cai < channel->avg_Ca_ExtraMsgNumber; cai++)
				Ca += CaArray[cai];
			Ca /= (double) (channel->avg_Ca_ExtraMsgNumber);
			channel->avg_Ca_ExtraMsgValue = Ca;
		}

		channel->avg_Ca = (1.0 - channel->post_const) * Ca
				+ channel->post_const * channel->avg_Ca;
		...
		if (channel->change_weights)
			changeStdpWeights(channel, weight_change_mod,
					weight_change_mod_level);
	...
	case RESET:
		...
		if (allocnodes != 0) {
			if (!channel->list_alloced) {
				if (AllocateSynapticEventNodes((struct Synchan_type *) channel,
						allocnodes, &(channel->FreeSynapticEvents), &lastnode)
						== 0)
					return (0);
	...
	case RECALC:
		...
		if (channel->pre_tau1 == channel->pre_tau2) {
			...
			channel->pre_yconst2 = exp(-dt / channel->pre_tau2) * (1.0 / a)
					* (1.0 - exp(-a * dt));
	...
	case RESETBUFFER:
		...
		/* need to allocate more nodes and link them in to the free list */
		{
			...
			if (AllocateSynapticEventNodes((struct Synchan_type *) channel,
					num_nodes, &firstnode, &lastnode) == 0)
				return (0);
		...
		if (AllocateSynapticEventNodes((struct Synchan_type *) channel,
				allocnodes, &(channel->FreeSynapticEvents), &lastnode) == 0)
			return (0);
	...
	case CHECK:
		...
		if (channel->nodes_per_synapse <= 0)
			ErrorMessage("HebbSynchan", "nodes_per_synapse must be > 0.",
					channel);
		...
		if (channel->pre_thresh_lo > channel->pre_thresh_hi)
			ErrorMessage("HebbSynchan",
					"pre_thresh_lo must be <= pre_thresh_hi.", channel);
		...
		if (channel->post_thresh_lo > channel->post_thresh_hi)
			ErrorMessage("HebbSynchan",
					"post_thresh_lo must be <= post_thresh_hi.", channel);
		...
		if (channel->min_weight > channel->max_weight)
			ErrorMessage("HebbSynchan", "min_weight must be <= max_weight.",
					channel);
		...
		for (i = 0; i < channel->nsynapses; i++) {
			if ((channel->synapse[i].weight < channel->min_weight)
				...
				ErrorMessage("HebbSynchan",
						"synaptic weights are not within [min_weight,max_weight] range.",
						channel);
			}
		}
	...
	case ADDMSGIN:
		...
		if (channel->synapse == NULL)
			channel->synapse = (HebbSynapse *) calloc(channel->nsynapses,
					sizeof(HebbSynapse));
		else {
	...
	case DELETEMSGIN:
		...
		if (synindex < 0) {
			ErrorMessage("HebbSynchan",
					"Could not find SPIKE message to delete.", channel);
			return (0);
		}
		...
		if (synindex != channel->nsynapses - 1) {
			RemapMsgData(channel, (char*) &channel->synapse[synindex + 1],
		...
		for (event = channel->PendingSynapticEvents; event != NULL; event =
				next_event) {
	...
	case COPY:
		...
		if (channel->synapse != NULL) {
			Error();
			printf(
					"hebbsynchan: COPY: cannot copy synchans with SPIKE messages in current implementation.\n");
			printf(
					"           Please add SPIKE messages after copying synchan objects, not before!\n");
	...
	case RESTORE2:
		...
		if ((int) savedata[0] != channel->nsynapses) {
			ErrorMessage("HebbSynchan",
					"Synapse count in savedata does not match", channel);
			return (n);
		}
	...
	case SET:
		return Synchan_SET("hebbsynchan", (struct Synchan_type *) channel,
				action);
		/*NOTREACHED*/
		break;

	case SHOW:
		return Synchan_SHOW("hebbsynchan", (struct Synchan_type *) channel,
				action);
		/*NOTREACHED*/
		break;

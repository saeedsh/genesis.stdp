...
static void changeStdpWeights( channel, weight_change_mod, weight_change_mod_level )
     ...
     double weight_change_mod_level;
{
  ...
  if ( channel->avg_Ca > channel->post_thresh_hi )
    post_activity = ( channel->avg_Ca - channel->post_thresh_hi ) / channel->post_scale;
  else if ( channel->avg_Ca < channel->post_thresh_lo )
    post_activity = ( channel->avg_Ca - channel->post_thresh_lo ) / channel->post_scale;
  else 
    post_activity = 0.0;
     ...
     Action  *action;
{
  ...
  SELECT_ACTION( action )
	...
	changeStdpWeights( channel, weight_change_mod, weight_change_mod_level );
